...
**Repository:** `GlacierEQ/sourcegraph-code-nav-intent-gate`  
**Company lens (independent):** Sourcegraph (`sourcegraph`)  
**Innovation:** Code Nav Intent Gate  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Sourcegraph affiliation, proprietary access, or production deployment.
...
Bind navigation queries to declared intent and hop budgets; refuse unbounded graph walks.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Sourcegraph employment, endorsement, or proprietary systems access.
...
File: `src/code_nav_intent_gate.py`

- Expand `CodeNavIntentGate` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_code_nav_intent_gate.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_code_nav_intent_gate.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Sourcegraph APIs
